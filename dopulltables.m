function [dataa,datas,dataa1,datas1]=dopulltables(p,simdir)
    ...
    datas1=dataa;
    for ii=1:size(p,1)/2
        [dataa(:,ii), datas(:,ii), dataa1(:,ii), datas1(:,ii)]=...
        pulltables([id p{ii*2,1} sprintf('_%d',p{ii*2,strcmp(p(1,:),'intnum')})], ...
            timesteps,steps_year,labels);
    end
    [dataa(:,end),datas(:,end),dataa1(:,end),datas1(:,end)]=pulltables([id 'BaselineInt'],timesteps,steps_year,labels);
